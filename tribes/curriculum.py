...
class Stage:
    ...
    name: str
    n_tribes: int
    opp_type: str  # "random" | "simple" | "mixed" | "self"
...
#: Default tribe pool – ordered by increasing board size contribution.
...
#: Curriculum: random → mixed bridge → simple → self-play, each with 2→4 tribes.
DEFAULT_CURRICULUM: list[Stage] = [
    # Phase 1 – Random opponents, growing map
    Stage("random-2p", 2, "random", 20_000, 0.55),
    Stage("random-3p", 3, "random", 30_000, 0.50),
    Stage("random-4p", 4, "random", 40_000, 0.45),
    # Phase 1.5 – Bridge: mix of random and simple, so the agent is gradually
    # exposed to purposeful play before facing a pure heuristic opponent.
    Stage("mixed-2p", 2, "mixed", 20_000, 0.45, mix_ratio=0.5),
    Stage("mixed-3p", 3, "mixed", 30_000, 0.40, mix_ratio=0.5),
    # Phase 2 – Heuristic (SimpleAgent) opponents, growing map.
    # Thresholds are lower than random stages because SimpleAgent is significantly
    # stronger; 45 % win rate already indicates a competent policy.
    Stage("simple-2p", 2, "simple", 40_000, 0.45),
    Stage("simple-3p", 3, "simple", 50_000, 0.40),
    Stage("simple-4p", 4, "simple", 60_000, 0.35),
    # Phase 3 – Self-play, growing map
    Stage("self-2p", 2, "self", 50_000, 0.55),
    Stage("self-3p", 3, "self", 60_000, 0.50),
    Stage("self-4p", 4, "self", 0, 0.0),  # final stage, no advancement
]
