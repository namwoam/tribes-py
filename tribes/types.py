...
class TRIBE(Enum):
    ...
    def __init__(
        ...
        key: int,
        name: str,
        initial_tech_name: Optional[str],
...
class RESOURCE(Enum):
    ...
    WHALES = (
        3,
        "img/resource/whale2.png",
        "img/resource/whale3.png",
        "w",
        0,
        10,
        "WHALING",
    ...
    def __init__(
        ...
        key: int,
        image_file: str,
        secondary_image_file: Optional[str],
        map_char: str,
        cost: int,
        bonus: int,
...
class BUILDING(Enum):
    ...
    def __init__(
        self, key: int, image_file: str, tech_name: Optional[str], terrain_names: set
    ) -> None:
        self._key = key
        self._image_file = image_file
        self._tech_name = tech_name
    ...
    def init_monuments_from_dict(data: dict) -> dict:
        ...
        for k, v in data.items():
            result[BUILDING.get_type_by_key(int(k))] = MONUMENT_STATUS.get_type_by_key(
                int(v)
            )
        return result
...
class EXAMINE_BONUS(Enum):
    ...
    def random(rng: "random.Random") -> EXAMINE_BONUS:
        ...
        return (
            _random.choice(bonuses)
            if rng is None
            else bonuses[rng.randint(0, len(bonuses) - 1)]
        )
...
class CITY_LEVEL_UP(Enum):
    ...
    def valid_type(self, city_level: int) -> bool:
        if city_level == 1 and self in (CITY_LEVEL_UP.WORKSHOP, CITY_LEVEL_UP.EXPLORER):
            return True
        if city_level == 2 and self in (
            CITY_LEVEL_UP.CITY_WALL,
            CITY_LEVEL_UP.RESOURCES,
        ):
            return True
        if city_level == 3 and self in (
            CITY_LEVEL_UP.POP_GROWTH,
...
class UNIT(Enum):
    ...
    MIND_BENDER = (
        7,
        "img/unit/mind_bender/",
        "img/weapons/effects/bender/",
        None,
        "PHILOSOPHY",
    )  # noqa: E221,E201
...
class ACTION(Enum):
    ...
    def __init__(
        self, _key: int, img_path: Optional[str], tech_name: Optional[str]
    ) -> None:
